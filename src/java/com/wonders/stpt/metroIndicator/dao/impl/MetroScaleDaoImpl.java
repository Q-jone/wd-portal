...
public class MetroScaleDaoImpl extends AbstractHibernateDaoImpl<MetroScale>
		implements MetroScaleDao {
	//按日期 线路 取得对应数据
	@SuppressWarnings("unchecked")
	public List<MetroScale> findLatestMetroScaleEvents(String date, String line){
		String hql = "from MetroScale m where m.removed = '0'";
		if(!"".equals(date)){
		...
		List<MetroScale> list = query.list();
		return list;
	}
	
	@SuppressWarnings("unchecked")
	public List<MetroScale> findScaleWithData(String endDate,String line){
		String hql = "from MetroScale t where t.removed = '0' and t.indicatorDate <= '"+endDate+"' and t.indicatorLine = '"+line+"' order by t.indicatorDate DESC";
	...
	public Page findMetroScaleByPage(Integer pageNo, int pageSize,String lineNo){
		...
		String hql = "select t from MetroScale t where t.indicatorLine='"+lineNo+"' ";
		String countHql = "select count(*) from MetroScale t where t.indicatorLine='"+lineNo+"' ";
		String filterPart = "";
	...
	public MetroScale findMetroScaleByDate(String date,String line){
		String hql = "from MetroScale t where t.indicatorDate='"+date+"' and t.indicatorLine='"+line+"'";
		Query query = getSession().createQuery(hql);
		return (MetroScale) query.uniqueResult();
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<MetroScale> findMetroScaleByDates(String[] dates, String line) {
		String hql = "from MetroScale t where t.removed ='0' and t.indicatorDate in (";
		if(dates.length>0){
	...
	public MetroScale findMetroScaleByLastDay(String date, String line) {
		String hql = "from MetroScale t where t.removed ='0' and t.indicatorDate like '%"+date+"%' and t.indicatorLine='"+line+"' order by t.indicatorDate DESC";
		Query query = getSession().createQuery(hql);
		query.setMaxResults(1);
		List<MetroScale> list = query.list();
	...
	public List<Object[]> findFirstAndLastDate() {
		String sql = "SELECT MIN(t.INDICATOR_DATE),MAX(t.INDICATOR_DATE) from DW_METRO_SCALE t";
	...
	public MetroScale findRecentlyScale(String date, String line) {
		String hql = "from MetroScale t where t.indicatorDate <='"+date+"' order by t.indicatorDate DESC";
		Query query = getSession().createQuery(hql);
		query.setMaxResults(1);
		List<MetroScale> list = query.list();
		if(list!=null && list.size()>0) return list.get(0);
