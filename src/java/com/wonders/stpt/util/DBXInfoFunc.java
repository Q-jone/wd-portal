...
public class DBXInfoFunc {
	
	
	public static List<Map<String,String>> findDBX(String loginName,String userName,String userDeptId,String size){
		String deptSql = "";
		deptSql = " and (instr(t.helpurl,'ST/" + loginName + ":"+userDeptId+"<+>') >0 or t.helpurl is null or instr(t.helpurl,'ST/" + loginName + ":<+>') >0) ";		
		String strSQL = "";
       
        strSQL=
        	" select inci.summary as summaryZS,inci.summary, "+
        	" '' as memo,"+
        	" inci.priorities, " +
        ...
        /**/
        strJobSQL=
        " select tjcg.summary as summaryZS, " +
		...
		strSQL = "select * from (("+strSQL+")union("+strJobSQL+")) where 1=1";
		strSQL = "select * from ( select * from ("+strSQL+")  order by endtime desc) where rownum<="+size;
		
		
		List<Map<String,String>> list = new ArrayList<Map<String,String>>();
		ExecuteSql dealsql= new ExecuteSql("dataSource2");
			try{
				ResultSet rs=dealsql.ExecuteDemandSql(strSQL);
	...
	//工作联系单
	public static List<Map<String,String>> findDYX(String userId, String loginName, String userName,String userDeptId,String size){
		
		 String dyxSQL = "";
		 String strSQL = "";
         dyxSQL = 
        	" select tjcg.summary as summaryZS, '<div class=summary groupid=' || twr.business_id ||' ptype=0 dbxtype=dyx>' || tjcg.summary || '</div> ' as summary," +
         ...
         strSQL = "select * from ( select * from ("+dyxSQL+")  order by endtime desc) where rownum<="+size;
 		ExecuteSql dealsql= new ExecuteSql("dataSource2");
         List<Map<String,String>> list = new ArrayList<Map<String,String>>();
