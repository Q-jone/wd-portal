...
package com.wonders.stpt.core.domainAuthentication.dao.impl;
...
public class DomainAuthenticationDaoImpl extends
		AbstractHibernateDaoImpl<DomainAuthentication> implements
		DomainAuthenticationDao {
	public Page findDomainAuthenticationByPage(Map<String, Object> filter,
			int pageNo, int pageSize) {
		if (filter == null)
			filter = new HashMap<String, Object>();
		List<HqlParameter> args = new ArrayList<HqlParameter>();
		String hql = "select t from DomainAuthentication t ";
		String countHql = "select count(*) from DomainAuthentication t ";
		String filterPart = "";
		int filterCounter = 0;
		if (!filter.isEmpty()) {
			filterPart += " where ";
