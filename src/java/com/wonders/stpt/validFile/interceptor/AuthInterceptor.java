...
* @version V1.0   
*/
package com.wonders.stpt.validFile.interceptor;
...
public class AuthInterceptor extends AbstractInterceptor{
	...
	*/
	...
	public String intercept(ActionInvocation invocation) throws Exception {
        ...
        String user = (String) session.get(LoginConstant.SECURITY_USER_NAME);
        t("begin");
        
        if (user != null && ("蔡伟东".equals(user) || "陈建东".equals(user)||"忻然".equals(user) || "朱英".equals(user))) {
            result = invocation.invoke();          
            System.out.print(invocation.getProxy().getMethod());
            System.out.print(invocation.getProxy().getActionName());
        }  else{
        	result = "no_authority_json";
        }
        t("end");
        return result;  
	}
