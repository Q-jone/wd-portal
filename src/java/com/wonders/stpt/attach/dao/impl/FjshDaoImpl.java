package com.wonders.stpt.attach.dao.impl;
...
import com.wonders.stpt.attach.dao.FjshDao;
...
@Repository("fjshDao")
public class FjshDaoImpl extends HibernateDaoSupport implements FjshDao {
	...
	public String getCurrentFileVersion(String groupId,String fileAllName){
		...
		try{
			if(!StringUtil.isNull(fileAllName)&&fileAllName.indexOf(".")>=0){
				...
				String sql = "select max(to_number(t.version)) from t_portal_attach t where t.appname=? and t.filename=? and t.fileextname=? and t.groupid=? and t.status != ?";
				
				int count = getJdbcTemplate().queryForInt(sql, new Object[]{AttachUtil.APP_MODEL_NAME,fileName,fileExtName,groupId,AttachUtil.STATUS_DELETE});
