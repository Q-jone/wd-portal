...
<%
	...
	String switchDeptUrl =SpringBeanUtil.getProperties("classpath:config.properties").getProperty("switchDeptUrl").trim();
	...
	if(urlCa!=null && !"".equals(urlCa)&&!"0".equals(caCross)){ 	
		...
		switchUrl = urlCa+"/changeDept.action?token="+token+"&appName="+caAppName+"&returnUrl=http://10.1.48.30/portal";
	}else{
		switchUrl = "/portal/switchDept.jsp?tmp=1";
	} */
	String switchUrl = "/portal/switchDept.jsp?tmp=1";
 	...
 	$.ajax({
		...
		$("li[id=tabs] a").click(function(){
			...
			if(tar == "_blank"){
				...
				var bottomFrameValue = $("#bottomFrame",parent.document).attr("src");
				if(bottomFrameValue == "bottom.jsp" && 
						mainPath!=null && mainPath.indexOf("xw_index") >= 0){
					$("#bottomFrame",parent.document).attr("src","bottomNull.jsp");					
				}else if(bottomFrameValue == "bottomNull.jsp"){
					$("#bottomFrame",parent.document).attr("src","bottom.jsp");
				}
...
<body onLoad="MM_preloadImages('../css/default/images/index_1_04_open.png')">
<input type="hidden" name="userId" id="userId" value="<s:property value="#session.userId"/>">
<input type="hidden" name="loginName" id="loginName" value="<s:property value="#session.loginName"/>">
<input type="hidden" name="deptId" id="deptId" value="<s:property value="#session.deptId"/>">
    <header class="mw1002">
   ...
   <%
	...
	MenuResource parentMenu = menuService.getTopMenuInfoByName("申通综合业务协同平台");
	List navTopMenu = Arrays.asList(menuService.getAuthMenusByParentMenu(parentMenu, userId));
	for (int i = 0; i < navTopMenu.size(); i++) {
		...
		String resourceCode = oneNavTopMenu.getCode();
		String url = "/portal/mainFrame/leftt.jsp?topid="+ resourceId;
		//String url = "/portal/mainFrame/left.html";
		String linkPath = oneNavTopMenu.getLinkPath();
		String target = oneNavTopMenu.getTarget();
		if(linkPath==null) linkPath = "";
	%>
            	<li id="tabs"><a style="cursor:pointer;" href="javascript:void(0);" code="<%=resourceCode%>" leftLink="<%=url%>" mainLink="<%=linkPath%>" winTarget="<%=target %>"><span><%=resourceName%></span></a></li>
         <%} %>  
            </ul>       
