...
$(function () {
    var treaty = {
        host:"http://10.1.48.16:7001",
        init:function(){
            this.loadTreatyList(this.host+"/investCost/api/contract/getTreatyList.action?callback=?");
        },
        wrapMarquee:function(len){
            $('.wrap').marquee({
                auto: true,
                interval: 2000,
                showNum: len>5?5:len,
        ...
        loadTreatyList:function(url){
            $.ajax({
                ...
                success: function (rtn) {
                    var list = rtn.list;
                    //var more = this.host + "/treaty/findTreatyByPage.action";
                    if (list) {
                        $.each(list, function (index, obj) {
                            var $li = $("<li ><a style='cursor:pointer;' target='_blank'></a></li>");
                            if (obj.buildSupplierName != "") {
                                $li.find("a").text(obj.buildSupplierName);
                                $li.find("a").attr("href", treaty.host + "/investCost/treaty/report.action?buildSupplierName="+obj.buildSupplierName);
                                $("#treatyList").append($li);
                            }
                        });
                        if (list.length == 0) {
                            $("#treatyList").append("&nbsp;&nbsp;无相关信息。");
                        }else{

                            treaty.wrapMarquee(list.length);
                        }
