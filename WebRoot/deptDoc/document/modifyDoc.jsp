...
<script type="text/javascript">
  	...
  	var handleOptions = {
			...
			type:'post',
			callback:null,
			url:'/portal/zdocsFile/updateDoc.action',
...
<body>
	<s:form theme="simple" action="updateDoc" id="handleForm" method="post" namespace="/zdocsFile">
		<input type="hidden" name="fileId" id="fileId" value="${zf.sid}"/>
		<table class="toolTbl">
        <tr>
            <td><font class="redMark">*&nbsp;</font>文件标题：</td>
            <td><input class="input_xlarge" type="text" name="fileName" id="fileName" maxlength="500" value="${zf.fileName}"/></td>
        </tr>
        <tr>
            <td><font class="redMark">*&nbsp;</font>主题词：</td>
            <td><input class="input_xlarge" type="text" name="keyword" id="keyword" maxlength="500" value="${zf.keyword}"/></td>
        </tr>
        </table>
        <br>
        <iframe scrolling="auto" height="200" frameborder="1" border="1" marginheight="0" marginwidth="0" width="100%" id="attachFrame" name="attachFrame" src="/portal/attachOld/loadFileOldList.action?fileGroup=attachGroup&fileGroupName=attachGroupDoc&fileGroupId=${zf.refId}&userName=<s:property value="#session.userName"/>&loginName=<s:property value="#session.loginName"/>&procType=edit&targetType=frame&type=1&fileCntObjId=attachCnt"></iframe>
