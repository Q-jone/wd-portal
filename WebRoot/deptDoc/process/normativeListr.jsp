	...
	<script type="text/javascript">
		...
		if(data!=null && data.list.length >0){
			...
			for(var i=0;i<l.length;i++){
				...
				temp += "<td style='cursor:pointer;'";
				temp += " vid='"+$.notNull(l[i].VID)+"' class='t_c showRemark'>"+($.notNull(l[i].REMARK).length>0?"<img alt='附件' src='<%=path%>/operation/ui/images/fj.gif'/>":"") +"</td>";
				temp += "<td class='t_c'>";
				if($.notNull(l[i].URL) == ""){
                    temp += " </td>";
                }else{
		...
		var listOptions = {
			...
			type:'post',
			callback:null,
			url:'/portal/deptDoc/process/normativeList.action?random='+Math.random(),
      ...
      <div class="pt45">
        <div class="tabs_2">
            <ul>
               ...
               <li><a href="<%=path%>/deptDoc/process/redvListr.jsp"><span>呈批件</span></a></li>
               <li><a href="<%=path%>/deptDoc/process/sendListr.jsp"><span>行政发文</span></a></li> 
               <li class="selected"><a href="<%=path%>/deptDoc/process/normativeListr.jsp"><span>规范性文件</span></a></li>  
        				...
        				<thead>
                              <tr class="tit">
                                ...
                                <th style="white-space:nowrap;" class="t_c">文件状态</th>
                                 ...
                                 <th style="white-space:nowrap;" class="t_c">操作</th>
                              </tr>
