...
<%
		...
		<tr>
			...
			extName = attachUtil.getFileImageName(fileExtName); %>
				...
				<a href="<%=request.getContextPath()%>/attach/downloadFile.action?fileId=<%=file.getAttachFile().getId()%>" target="_blank" style="display:inline;"><%=file.getFileAllName() %></a>&nbsp;
				<%
			...
			<%
			 ...
			 %>
			 <td style="line-height: 15px"><a style="display:inline;" href="#" onclick="javascript:loadVersionFiles('<%=file.getGroupId() %>','<%=file.getAttachFile().getId() %>')" >v<%=file.getVersion() %></a><input type="button" name="str" value="历" onclick="javascript:loadVersionFiles('<%=file.getGroupId() %>','<%=file.getAttachFile().getId() %>')"></td>
			 <%} %>
			...
			<!--<td style="word-break:break-all">
				...
				%>
					<a style="display:inline;" href="<%=request.getContextPath()%>/attach/pdfPreview.action?fileId=<%=file.getAttachFile().getId()%>" target="_blank">
				...
				%>
					<a style="display:inline;" href="<%=request.getContextPath()%>/attach/imagePreview.action?fileId=<%=file.getAttachFile().getId()%>" target="_blank">
					<img src="<%=request.getContextPath()%>/css/default/images/files/find.gif" />
					</a>
				<%
					}
