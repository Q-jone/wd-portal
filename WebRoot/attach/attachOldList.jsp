...
<%@page import="com.wonders.stpt.attach.model.vo.UploadFile,com.wonders.stpt.dataExchange.model.bo.OldAttachFile,com.wonders.stpt.attach.util.AttachUtil"%>
...
<%
		for(int i=0;i<fileList.size();i++){
			OldAttachFile file = (OldAttachFile)fileList.get(i);
		%>
		<script type="text/javascript">
		...
		<tr>
			...
			extName = attachUtil.getFileImageName(fileExtName); %>
				...
				<a href="http://10.1.44.18/downloadFile.action?fileId=<%=file.getId()%>" target="_blank" style="display:inline;"><%=file.getFileName() %>.<%=file.getFileExtName() %></a>&nbsp;
				<%
			...
			<%
			 ...
			 %>
			 <td style="line-height: 15px"><a style="display:inline;" href="#" onclick="javascript:loadVersionFiles('<%=file.getGroupId() %>','<%=file.getId() %>')" >v<%=file.getVersion() %></a><input type="button" name="str" value="历" onclick="javascript:loadVersionFiles('<%=file.getGroupId() %>','<%=file.getId() %>')"></td>
			 <%} %>
			<!--td style="line-height: 15px"><a href="<%=request.getContextPath()%>/attach/downloadFile.action?fileId=<%=file.getId()%>" target="_blank">下载</a></td>
			<%
			if(StringUtil.isNull(procType)||!procType.equals("view")){	//不可操作模式
			 %>
			<td style="line-height: 15px"><a href="<%=request.getContextPath()%>/attach/deleteFilesJsp.action?deleteData=<%=file.getId()%>&fileGroup=<%=fileGroup%>&fileGroupId=<%=fileGroupId%>&fileGroupName=<%=fileGroupName%>"><font color=red>删除</font></a></td>
			<%} %>
			-->
			<td style="word-break:break-all">
				<%
					String memo=file.getMemo();
					memo=(memo==null)?"":memo;
					out.println(memo);
				%>&nbsp;
			</td>
...
<%-- 			<td style="word-break:break-all">
				...
				%>
					<a style="display:inline;" href="<%=request.getContextPath()%>/attach/pdfPreview.action?fileId=<%=file.getId()%>" target="_blank">
				...
				%>
					<a style="display:inline;" href="<%=request.getContextPath()%>/attach/imagePreview.action?fileId=<%=file.getId()%>" target="_blank">
					<img src="<%=request.getContextPath()%>/css/default/images/files/find.gif" />
					</a>
				<%
					}
