...
<!--[if IE 6.0]>
	...
	jQuery.notNull = function(options){
		...
		var listOptions = {
			...
			type:'post',
			callback:null,
			url:'/portal/applyProjectList/projectList.action?random='+Math.random(),
		    	...
		    	$("#checkboxAll").prop("checked",false);
					...
					if(data!=null && data.list.length >0){
						...
						for(var i=0;i<l.length;i++){
							...
							temp += "<td class='t_l'>"+$.notNull(l[i].PROJECTNAME) +"</td>";
							temp += "<td class='t_c'>"+$.notNull(l[i].PROJECTSTATUS) +"</td>";
							temp += "<td class='t_l'>"+$.notNull(l[i].PROJECTACHIEVEMENT).replace(/\n/g,"<br>") +"</td>";
							temp += "<td class='t_l'>"+$.notNull(l[i].PROJECTPROGRESS).replace(/\n/g,"<br>") +"</td>";
							temp += "<td class='t_l'>"+$.notNull(l[i].PROJECTREMARK).replace(/\n/g,"<br>") +"</td>";
							...
							temp += "<td class='t_c'><a class='editA' href='javascript:void(0);' projectId='"+l[i].ID+"'>编辑</a></td>";
							</s:if>
							temp += "<td class='t_c'><a target='_blank' href='/portal/applyProject/projectView.action?id="+l[i].ID+"'>查看</a></td>";
							temp += "</tr>";
						}
	 	...
	 	$(function(){
			...
			$(document).on("click",".clk",function(){
	 		...
	 		$("#addButton").click(function(){
	 			openWinn("/portal/applyProject/project/add.jsp",new Date().getTime());
	 		})
	 		
	 		$(document).on("click",".editA",function(){
	 			openWinn("/portal/applyProject/projectEdit.action?id="+$(this).attr("projectId"),$(this).attr("projectId"));
	 			return false;
	 		})
      ...
      <div class="pt45">
        <div class="tabs_2">
        	<ul>
	           	...
	           	<li><a href="/portal/applyProject/accident/list.jsp"><span>事件管理</span></a></li>
	        	<li class="selected"><a href="/portal/applyProject/project/list.jsp"><span>项目管理</span></a></li>
        	...
        	<input type="hidden" name="page" id="page" value=""/>
        	 <table width="100%" border="0" cellspacing="0" cellpadding="0">
        	    <tr>
        	     ...
        	     <td class="t_r"></td>
        	     <td><input type="hidden" id="type_equal" name="type_equal" value="3"/></td>
        	     <td class="t_r"></td>
		      ...
		      <div class="fn clearfix">
		                  <h5 class="fl"><a href="#" class="colSelect fl">项目管理列表</a></h5>
		                  <s:if test='#session.userName=="胡波" || 
					        	#session.userName=="李名敏" ||
					        	#session.userName=="金涛" ||
					        	#session.userName=="黄天印"'>
        	...
        	<table width="100%"  class="table_1" id="mytable">
        				<thead>
                              <tr class="tit">
                                ...
                                <th style="white-space:nowrap;" class="t_l">项目名称</th>
                                <th style="white-space:nowrap;" class="t_c">状态</th>
                                <th style="white-space:nowrap;" class="t_l">已有成果</th>
                                <th style="white-space:nowrap;" class="t_l">当前进展</th>
                                <th style="white-space:nowrap;" class="t_l">备注</th>
