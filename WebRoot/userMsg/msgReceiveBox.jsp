...
<ul id="msgTab" class="nwarp">
	<li  class="selected" id="rbox"><a href="javascript:void(0);"><span>收件箱</span></a></li>
	<li id="sbox"><a href="javascript:void(0);"><span>发件箱</span></a></li>
	<li id="dbox"><a href="javascript:void(0);"><span>草稿箱</span></a></li>
</ul>
</div>
<!--Filter-->
<div class="filter">
<div class="query">
<div class="p8 filter_search"><s:form action="msgReceiveBox"
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			...
			<td><input value="<s:property value="msgBo.sname"/>" type="text"
				id="sname" name="sname" class="input_xxlarge" /></td>
		</tr>
		<tr>		
			<td class="t_r">发送时间：</td>
			<td><input id="sendStart"
				value="<s:property value="#request.sendStart"/>" name="sendStart"
				type="date" class="input_medium" readonly="readonly" />   至       <input
				id="sendEnd" value="<s:property value="#request.sendEnd"/>"
				name="sendEnd" type="date" class="input_medium" readonly="readonly" />
			</td>
			<td class="t_r">状态：</td>
			<td>
				<select name="seeState" id="seeState" class="input_medium">
					<option value="">---全部---</option>
					<option value="0" <s:if test='msgBo.seeState=="0"'> selected </s:if>>未读</option>
					<option value="1" <s:if test='msgBo.seeState=="1"'> selected </s:if>>已读</option>
				</select>
			</td>
		</tr>
		<tr>
...
<h5 class="fl"><a href="#" class="colSelect fl">收件箱</a></h5>&nbsp;
...
<input type="button" onclick="delBatch('receive');" value="批量删除" class="fr mr5">
...
<table width="100%" class="table_1">
	<tbody id="pageTable">
		...
		<s:iterator value="pageResultSet.list" id="msg">
			<tr <s:if test='#msg.seeState=="0"'> class="bd" </s:if>>
				<td class="t_c"><input type="checkbox" id="delId" name="delId" value="<s:property value='#msg.id'/>"/></td>
				<td class="mail_1 <s:if test='#msg.replyId!=null'> mail_re </s:if><s:elseif test='#msg.forwardId!=null'> mail_for </s:elseif><s:elseif test='#msg.seeState=="0"'> mail_new </s:elseif><s:elseif test='#msg.seeState=="1"'> mail_read </s:elseif>">&nbsp;</td>
				...
				<td class="t_c"><a class="mr5" href="msg<s:if test='#msg.seeState=="0"'>Read</s:if><s:else>View</s:else>.action?msgId=<s:property value="#msg.id"/>" target="_self">查看</a></td>
			</tr>
		</s:iterator>
	</tbody>
	<tr class="tfoot">
		<td style="border-top: #5a84a7 2px solid;" colspan="6">
