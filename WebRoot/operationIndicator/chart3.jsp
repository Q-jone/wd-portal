...
function findData(){
	...
	if(lineStatus==-1){
		$.ajax({
			...
			success:function(object){
				if(object==null || object==""){
					...
					if(object[object.length-1].metroProductionControlVO!=null){
						for(var i=0; i<object.length; i++){
							if(object[i].metroProductionControlVO!=null){
								controlValue = object[i].metroProductionControlVO.passengerCapacityDaily;
							...
							dataList[i] = object[i].passengerCapacityDaily;
					...
					newChartColumn(dataList,dateList,'chart3','单位：万人/日',getInterval(),controlList);
				}
			}
		});
	}else{
		...
		window.location.href="showProductionDetailPage.action?lineNo="+$("#lineNoSelect").val()+"&startDate="+startDate+"&endDate="+endDate+"&chartId=3"
	}
}
		...
		<table width="100%" class="table_1">
			...
			<s:iterator value="#request.page.result" id="pm" >
				<tr>
					...
					<td id="td_3">
						<s:if test="#pm.passengerCapacityDaily!=null && #pm.passengerCapacityDaily!='0.0'">
							<s:property value="#pm.passengerCapacityDaily"/>&nbsp;
						</s:if>
						<s:else>0&nbsp;</s:else>
					</td>
					<td id="td_3"><s:property value="#pm.lastyearMetroProduction.passengerCapacityDaily"/>&nbsp;</td>
					<td id="td_4"><s:property value="#pm.passengerCapacityMonth"/>&nbsp;</td>
					<td id="td_5"><s:property value="#pm.passengerCapacityYear"/>&nbsp;</td>
				</tr>
			</s:iterator>
