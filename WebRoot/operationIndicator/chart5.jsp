...
function findData(){
	...
	if(lineStatus==-1){
		$.ajax({
			...
			success:function(object){
				if(object==null || object==""){
					...
					if(object[object.length-1].metroProductionControlVO!=null){
						for(var i=0; i<object.length; i++){
							if(object[i].metroProductionControlVO!=null){
								controlValue = object[i].metroProductionControlVO.ticketIncomeDaily;
							...
							dataList[i] = object[i].ticketIncomeDaily;
					...
					newChartColumn(dataList,dateList,'chart5','单位：万元',getInterval(),controlList);
				}
			}
		});
	}else{
		...
		window.location.href="showProductionDetailPage.action?lineNo="+$("#lineNoSelect").val()+"&startDate="+startDate+"&endDate="+endDate+"&chartId=5"
	}
}
			...
			<tbody id="tbody">
				...
				<s:iterator value="#request.page.result" id="pm" >
					<tr>
						...
						<td id="td_3">
							<s:if test="#pm.ticketIncomeDaily!=null && #pm.ticketIncomeDaily!='0.0'">
								<s:property value="#pm.ticketIncomeDaily"/>&nbsp;
							</s:if>
							<s:else>0</s:else>
						</td>
						<td id="td_3">
							<s:if test="#pm.lastyearMetroProduction.ticketIncomeDaily=!null && #pm.lastyearMetroProduction.ticketIncomeDaily!='0.0'">
								<s:property value="#pm.lastyearMetroProduction.ticketIncomeDaily"/>&nbsp;
							</s:if>
							<s:else>0</s:else>
						</td>
						<td id="td_4"><s:property value="#pm.ticketIncomeMonth"/>&nbsp;</td>
						<td id="td_5"><s:property value="#pm.ticketIncomeYear"/>&nbsp;</td>
					</tr>
				</s:iterator>
			</tbody>
