...
function findData(){
	if(lineStatus==-1){
		$.ajax({
			type : 'post',
			url : 'findQualityBetweenDays.action?random='+Math.random(),
			dataType:'json',
			cache : false,
			data:{
			...
			success:function(object){
				if(object==null || object==""){
					...
					if(object==null){
						...
						var controlValue ;
						if(object[object.length-1].metroQualityControlVO!=null){
							controlValue = object[object.length-1].metroQualityControlVO.metroOnworkDaily;
							for(var i=0; i<object.length; i++){
								dataList[i] = object[i].metroOnworkDaily == 0 ? null : object[i].metroOnworkDaily;
								dateList[i] = object[i].indicatorDate.substr(5);
								controlList.push(parseFloat(controlValue));
							}
						}else{
							for(var i=0; i<object.length; i++){
								dataList[i] = object[i].metroOnworkDaily == 0 ? null : object[i].metroOnworkDaily;
						...
						newChartLine(dataList,dateList,'chart9',getInterval(),controlList);
					}
				}
			}
		});
		...
		window.location.href="showQualityDetailPage.action?lineNo="+$("#lineNoSelect").val()+"&startDate="+startDate+"&endDate="+endDate+"&chartId=9"
	}
}
...
<input id="lastDate" value="<s:property value='#request.lastDate'/>" type="hidden"/>
			...
			<tbody id="tbody">
				...
				<s:iterator value="#request.page.result" id="pm" >
					<tr>
						...
						<td id="td_3"><s:property value="#pm.metroOnworkDaily"/>&nbsp;</td>
						<td id="td_3"><s:property value="#pm.lastyearQualityVo.metroOnworkDaily"/>&nbsp;</td>
						<td id="td_3"><s:property value="#pm.metroOnworkMonth"/>&nbsp;</td>
						<td id="td_3"><s:property value="#pm.metroOnworkYear"/>&nbsp;</td>
					</tr>
				</s:iterator>
			</tbody>
	...
	<s:form action="showQualityDetailPage" name="qualityIndicator" namespace="/operationIndicator" id="form">
