...
function findData(){
	...
	if(lineStatus==-1){
		$.ajax({
			...
			success:function(object){
				if(object==null || object==""){
					...
					if(object[object.length-1].metroProductionControlVO!=null){
						for(var i=0; i<object.length; i++){
							...
							if(object[i].metroProductionControlVO!=null){
								controlValue = object[i].metroProductionControlVO.passengerTransferDaily;
							...
							dataList[i] = object[i].passengerTransferDaily;
					...
					newChartColumn(dataList,dateList,'chart4','单位：万人/日',getInterval(),controlList);
				}
			}
		});
	}else{
		...
		window.location.href="showProductionDetailPage.action?lineNo="+$("#lineNoSelect").val()+"&startDate="+startDate+"&endDate="+endDate+"&chartId=4"
	}
}
			...
			<tbody id="tbody">
				...
				<s:iterator value="#request.page.result" id="pm" >
					<tr>
						...
						<td id="td_3">
							<s:if test="#pm.passengerTransferDaily!=null && #pm.passengerTransferDaily!='0.0'">
								<s:property value="#pm.passengerTransferDaily"/>&nbsp;
							</s:if>
							<s:else>0&nbsp;</s:else>
						</td>
						<td id="td_3"><s:property value="#pm.lastyearMetroProduction.passengerTransferDaily"/>&nbsp;</td>
						<td id="td_4">
							<s:if test="#pm.passengerTransferMonth!=null && #pm.passengerTransferMonth!='0.0'">
								<s:property value="#pm.passengerTransferMonth"/>&nbsp;
							</s:if>
							<s:else>0&nbsp;</s:else>
						</td>
						<td id="td_5"><s:property value="#pm.passengerTransferYear"/>&nbsp;</td>
					</tr>
				</s:iterator>
			</tbody>
