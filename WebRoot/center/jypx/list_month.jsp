...
function save(){
	...
	$('.dataRow').each(function(){
		...
		if($("td:eq(2) select",tr).length == 0){
			month = $("td:eq(2)",tr).text();
		}else{
			month = $("td:eq(2) select option:selected",tr).text();
		}
		if(map[month]){
			duplicate = true;
			return false;
	...
	if(duplicate){
		alert('您添加了重复的月份，请删除重复月份后再提交！');
		return false;
	}		
	p += 'num='+i;	
	$.ajax({
		type: 'POST',
		url: '/portal/train/saveMonth.action?random='+Math.random(),
		data: p,
		dataType:'json',
		cache : false,
...
function doDel(id){
	 $.ajax({
	        type: "post",
	        url: "/portal/train/delMonth.action",
	        dataType: "json",
        ...
        <div class="gray_bg">
        	...
        	<form>
            	<div class="w_bg">
                	<div class="Bottom">
                    	<div class="Top">
                        	<h1 class="t_c">上海申通地铁集团有限公司<br>培训及鉴定月度结果录入
                       	  </h1>
                            <div class="mb10">
                           	  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_2" id="table">
                           	 	<tr class="disable">
        <td class="lableTd" colspan="6" style="text-align:center">培训中心月度培训及鉴定结果录入<span class="fr pt5"><a href="#" id="add"><img src="../css/default/images/+.png" width="15" height="15"></a></span></td>
                      		</tr>
					<tr class="disable">
        				<td class="lableTd" style="width:40px;">年度</td>
        				<td class="lableTd" style="width:80px;">类别</td>
        				<td class="lableTd" style="width:80px;">月度</td>
        				<td class="lableTd" >当月培训完成人次</td>
        				<td class="lableTd" >当月鉴定人次</td>
        				<td class="lableTd" >当月鉴定合格人次</td>
					</tr>
					<s:iterator value="#request.months" id="data" status="s">					       
                     <tr class="disable dataRow" >
                     	...
                     	<td class="lableTd"><s:property value="#data.month" /></td>
                     	<td class="lableTd"><input type="text" name="trained" style="width:80px;" onblur="checkNum(this)" value="<s:property value="#data.trained" />" maxlength="9"/></td>
                     	...
                     	<td class="lableTd">
                     		<input type="text" name="passed" style="width:80px;" onblur="checkNum(this)" value="<s:property value="#data.passed" />" maxlength="9"/>
                     		<span class="fr pt5"><a href="#" class='remove'><img src="../css/default/images/-.png" width="15" height="15"/></a></span>
                     	</td>
					</tr>
					</s:iterator>
                    <tr class="disable" id="dataNew" style="display:none;">
                     	...
                     	<td class="lableTd"><input type="text" name="trained" style="width:80px;" onblur="checkNum(this)" value="0" maxlength="9"/></td>
