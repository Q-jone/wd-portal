...
function handleFunc(obj,choice){
     ...
     var personalPrizeIds = '', teamPrizeIds = '', projectPrizeIds = '', achivementPrizeIds = '';
     $("input[name='check_personal']:checked").each(function(){
    	 personalPrizeIds += ','+this.value;
     });
     $("input[name='check_team']:checked").each(function(){
    	 teamPrizeIds += ','+this.value;
     });
     $("input[name='check_project']:checked").each(function(){
    	 projectPrizeIds += ','+this.value;
     });
     $("input[name='check_achivement']:checked").each(function(){
    	 achivementPrizeIds += ','+this.value;
     });             
     if(personalPrizeIds == '' && teamPrizeIds == '' && projectPrizeIds == '' && achivementPrizeIds == ''){
         alert('请在已申报奖项中勾选要退回的！');
		 $("#handle_zone_loading").hide();
		 closeMessageBox("handle_zone","maskDiv");
         return;
     }
     
     personalPrizeIds = personalPrizeIds.length>0?personalPrizeIds.substr(1):personalPrizeIds;
     teamPrizeIds = teamPrizeIds.length>0?teamPrizeIds.substr(1):teamPrizeIds;
     projectPrizeIds = projectPrizeIds.length>0?projectPrizeIds.substr(1):projectPrizeIds;
     achivementPrizeIds = achivementPrizeIds.length>0?achivementPrizeIds.substr(1):achivementPrizeIds;
     
     var remark = $('[name=remark]').val();
     
	if(confirm("确定要退回选中的奖项吗？")){
 		$.post(
				'<%=path%>/unionPrize/rejectByIds.action?random='+Math.random(),
				{
					"id": '${match.id}',
					"personalPrizeIds": personalPrizeIds,
					"teamPrizeIds": teamPrizeIds,
					"projectPrizeIds": projectPrizeIds,
					"achivementPrizeIds": achivementPrizeIds,
					"remark": remark,
					"choice": '1'
				},
