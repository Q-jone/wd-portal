		...
		var formOptions1 = {
		    ...
		    success:function(data){
				if(data=="1"){
					...
					alert("您是部门内唯一在进行申报的人员，请将所有奖项名额申报后再提交！");
				}else if(data=="4"){
					alert("您没有申报奖项，请申报后再提交，或选择'我没有奖项要申报'！");
				}else if(data=="3"){
					alert("您已经申报过奖项，请删除后再提交！");
				}else{
...
function loadDeptLeaders(){
	$.post(
			'<%=path%>/unionOrgan/getDeptLeaders.action?deptId=${applyMatch.deptId}&random='+Math.random(),
			{},
