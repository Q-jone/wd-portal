...
<div class="main f_bg">
					...
					<iframe scrolling="auto" height="80" frameborder="1" border="1" marginheight="0" marginwidth="0" width="100%" id="attachFrame" name="attachFrame" src="/portal/attach/loadFileList.action?fileGroup=attach&fileGroupName=attachGroup&fileGroupId=${match.attach}&userName=<s:property value="#session.userName"/>&loginName=<s:property value="#session.loginName"/>&procType=view&targetType=frame&type=1"></iframe>
						...
						<tr class="tfoot">
							...
							<s:if test="#request.match.handlerId.substring(0,12)==#session.t_login_name">
	                            ...
	                            <s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_STATUS || #request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_SECOND_REVIEW_STATUS">
	                            	<s:set name="dealBtnName" value="'提 交'"/>
	                            </s:if>                            	
	                            <s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_ASSIGN_STATUS">
	                            	<s:set name="dealBtnName" value="'下 一 步'"/>
	                            </s:if>
	                            
                             	<input type="button" id="todo_handle" value="<s:property value="#dealBtnName"/>" id="addButton">&nbsp;
                             </s:if>
    ...
    <div class="filter">
        <div class="fn clearfix">
            ...
            <div class="fr">
            <s:if test="#request.match.handlerId.substring(0,12)==#session.t_login_name">
            	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_STATUS">
                	<input type="button" id="addPrizeBtn" onclick="showAddPage();" value="新 增 奖 项" style="margin-left: 5px"/>
                </s:if>
            	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_ASSIGN_STATUS">
                	<span style="color:#ff0000">请在设置完参赛单位后点击上方的“下一步”按钮</span>
                </s:if>                
			</s:if>
            </div>
        </div>
...
<%--                 <td class="t_c"> 
                ...
                <td class="t_c">
                	<s:if test="#request.match.handlerId.substring(0,12)==#session.t_login_name">
	                	...
	                	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_ASSIGN_STATUS">
	                    	<a class="mr5" style="display: inline;cursor:pointer;" onclick="modifyQuantity('${data.id}','${data.quantity}','true')">设置参赛单位</a>
	                    </s:if>
                    </s:if>
                	<s:if test="#request.match.status > @com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_ASSIGN_STATUS">
                    	<a class="mr5" style="display: inline;cursor:pointer;" onclick="modifyQuantity('${data.id}','${data.quantity}','false')">查看参赛单位</a>
                    </s:if>            
                    <a class="mr5" style="display: inline;cursor:pointer;" onclick="showView('${data.id}')">查看</a>        
                </td>
            </tr>
			</s:iterator>
            <tr>
	...
	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_STATUS">
		<jsp:include page="flow/prizeSet.jsp"/>
	</s:if>
	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_DEPT_REVIEW_STATUS">
		<jsp:include page="flow/prizeSet_deptReview.jsp"/>
	</s:if>	
	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_FIRST_REVIEW_STATUS">
		<jsp:include page="flow/prizeSet_firstReview.jsp"/>
	</s:if>	
	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_APPROVE_STATUS">
		<jsp:include page="flow/prizeSet_approve.jsp"/>
	</s:if>	
	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_SECOND_REVIEW_STATUS">
		<jsp:include page="flow/prizeSet_secondReview.jsp"/>
	</s:if>		
	<s:if test="#request.match.status==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_ASSIGN_STATUS">
		<jsp:include page="flow/prizeAssign.jsp"/>
	</s:if>	
        </form>
