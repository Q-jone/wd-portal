...
<script src="<%=path%>/js/flick/jquery.ui.datepicker-zh-CN.js"></script>
	<script type="text/javascript">
      ...
      //跳转到新增页面
        ...
        function showPrizeSetPage(matchId){
        	sonWindow = window.open('<%=path%>/unionPrize/list.action?matchId='+matchId);
        }      
        
        function showOperatorSetPage(matchId){
        	sonWindow = window.open('<%=path%>/unionPrize/list.action?matchId='+matchId+'&op=setOperator');
        }         
        	...
        	<table width="100%"  class="table_1">
                              ...
                              <tr>
                                ...
                                <s:if test="#data.HANDLER_ID.substring(0,12)==#session.t_login_name"> 
	                                <s:if test="#data.STATUS==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_STATUS">
	                                	<a class="mr5" style="display: inline;cursor:pointer;" href="javascript:showPrizeSetPage('<s:property value="#data.ID"/>')">奖项设置</a>
	                                </s:if>
	                                <s:if test="#data.STATUS==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_SET_SECOND_REVIEW_STATUS">
	                                	<a class="mr5" style="display: inline;cursor:pointer;" href="javascript:showPrizeSetPage('<s:property value="#data.ID"/>')">确认预算</a>
	                                </s:if>                                
	                                <s:if test="#data.STATUS==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_ASSIGN_STATUS">
	                                	<a class="mr5" style="display: inline;cursor:pointer;" href="javascript:showPrizeSetPage('<s:property value="#data.ID"/>')">名额分配</a>
	                                </s:if>        
	                                <s:if test="#data.STATUS==@com.wonders.stpt.union.entity.bo.UnionMatch@PRIZE_ASSIGN_OPERATOR_STATUS">
	                                	<a class="mr5" style="display: inline;cursor:pointer;" href="javascript:showOperatorSetPage('<s:property value="#data.ID"/>')">选择申报人</a>
	                                </s:if>                                                        
                                </s:if>
